#!/usr/bin/env python3
"""
Append and deduplicate Web of Science plain-text exports for LLM screening.
...
Archive behavior:
  ...
  reports/archive/YYYY-MM-DD/ after new records are appended to the JSON output.
  Archived files are never scanned again.

Main output:
  data/paper_items.json
...
def record_to_paper_item(raw: dict[str, str]) -> dict[str, str] | None:
...
def item_key(item: dict[str, Any]) -> str:
    return clean_text(item.get("Key") or item.get("key"))
...
def load_existing_items(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(data, list):
        raise RuntimeError(f"Existing output JSON must be a list: {path}")
...
def merge_wos_exports(
    ...
    stats = {
        ...
        "appended_records": len(appended_items),
        "duplicates_skipped": duplicates,
        "skipped_records_or_files": skipped,
    }
    return appended_items, stats, parsed_files
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Append and deduplicate WoS plain-text exports for LLM screening.")
    parser.add_argument(
        "--workspace",
        default=str(Path(__file__).resolve().parents[2]),
        help="Project root. Default: parent folder of skills/fetch-merge/.",
    )
    parser.add_argument("--input", action="append", default=[], help="Input folder or txt file. Can be repeated.")
    parser.add_argument("--out-json", default="data/paper_items.json", help="Output JSON path, relative to workspace unless absolute.")
    ...
    existing_items = load_existing_items(out_json)
    existing_keys = {item_key(item) for item in existing_items if item_key(item)}
    items, stats, parsed_files = merge_wos_exports(input_dirs, existing_keys=existing_keys)
    if items:
        write_json_list(out_json, existing_items + items)
    ...
    final_stats = {
        ...
        **archive_stats,
        ...
        "output_records": len(existing_items) + len(items),
    }
    print("WoS append complete")
